...
function drawRadar(
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Align left/right based on side
...
export async function generatePoster(session: AssessmentSession, filename: string): Promise<void> {
  ...
  // ── Identity line: "小明是张一鸣型创业者" or just persona name ──────────
  const nickname = session.nickname?.trim()
  if (nickname) {
    // Two-line layout: nickname / "是" + persona name
  ...
  const orderedScores: DimensionScore[] = DIMENSIONS.map(
    (dim) => dimensionScores.find((s) => s.code === dim.code) ?? { ...dim, rawScore: 0, normalizedScore: 0, rank: 6 },
  )
