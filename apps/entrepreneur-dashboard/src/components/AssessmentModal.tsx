...
import { X, ChevronRight, Sparkles, Brain, Target, Shield, Zap, TrendingUp, RefreshCw, Star } from 'lucide-react';
      ...
      >
        ...
        {step === 'intro' && (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <h2 className="text-3xl font-bold text-slate-800 mb-3">AI 深度创业人格测评</h2>
                  ...
                  <div className="space-y-3">
                    {QUESTIONS[currentIdx]?.options.map((option, i) => (
                      <button
                        ...
                        className="w-full p-5 bg-slate-50 border border-slate-100 rounded-2xl text-left hover:bg-white hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/8 transition-all group flex items-center justify-between gap-4"
                      >
        ...
        {step === 'loading' && (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-800 mb-2">正在构建你的人格模型…</h3>
        ...
        {step === 'result' && personaData && (
          <div className="flex-1 overflow-y-auto">
            <div className="bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 px-10 py-9 text-center">
              {nickname && (
                <p className="text-blue-400 text-xs font-black uppercase tracking-[0.15em] mb-2">{nickname}</p>
              )}
              <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-500/20 border border-blue-500/30 text-blue-300 text-[10px] font-black rounded-full uppercase tracking-widest mb-5">
                <Sparkles className="w-3 h-3" />
                Core Identity
              </div>
              <h2 className="text-4xl font-black text-white tracking-tight mb-2">{personaData.name}</h2>
              <p className="text-sm text-slate-400 leading-relaxed max-w-xs mx-auto">"{personaData.keyword}"</p>
            </div>

            <div className="p-8 space-y-5">
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5">能力雷达</p>
                <p className="text-xs text-slate-400 mb-4">基于你的答题决策路径计算</p>
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  {personaData.insights.map((insight, i) => {
                    ...
                    return (
                      <div key={i} className="p-4 bg-white border border-slate-100 rounded-2xl flex items-start gap-4 shadow-sm">
                        <div className={cn(
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500 leading-relaxed mt-1">{insight.desc}</p>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="bg-slate-900 rounded-2xl p-6 relative overflow-hidden">
                ...
                <div className="relative">
                  ...
                  <button
                    onClick={reset}
                    className="mt-5 px-5 py-2 bg-white/10 hover:bg-white/15 text-white/80 text-xs font-bold rounded-xl flex items-center gap-2 transition-colors"
                  >
